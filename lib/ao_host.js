...
class AOHost extends AsyncEventEmitter {
  ...
  constructor (args = {}) {
    ...
    this.triggerPositionEvent = this.triggerPositionEvent.bind(this)
  ...
  async triggerPositionEvent (section, eventName, position) {
    ...
    await PI.forEach(instances, async (instance) => {
      const { state = {} } = instance
      const { positions = {}, allPositions = {}, closedPositions = {}, id, gid } = state

      console.log('---YY--XX-->', positions, allPositions, closedPositions)
      ...
      if (position.status === 'CLOSED') {
        closedPositions[pid] = position
      }

      allPositions[pid] = position
